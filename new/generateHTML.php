        ...
        if( $element->columns == 'doubleColumn'){
            ...
            <div class="form-group row no-gutters" > <!--Standard form group with two inputs-->
                ...
                <div class="input-group col-sm-10" >'
                    .((is_array($element->attribute) == false)?'<input type="text" name="'. $element->prefix . $element->name .'" class="form-control form-control-sm inputField"' .(($element->value=='date')?'':'onfocus="expandInput(this)"') . 'id="'. $element->name .'_attribute_input" ' . ((is_array($element->attribute))?'value='. "'" . (string)$element->attribute[0] . "'" .'':'value='. "'" . (string)$element->attribute . "'" .'') . ' placeholder="Attribute"' . (($element->readOnly==true)?'disabled="disabled"':'') . ' ' . (($element->hidden==true)?'hidden="true"':'') . '>':generateDropdown($element)) .
                    '<input type="'.(($element->value=='date')?'date':'text').'" ' .(($element->value=='date')?'':'onfocus="expandInput(this)"') .' class="form-control form-control-sm inputField" id="'. $element->name .'_value_input" placeholder="Value" value="">
                </div>
            </div>';
        ...
        //Generate html element with two input fields for xml attribute and value and a button to dynamically add additional copies of this element
            ...
            <div class="form-group row no-gutters justify-content-end"> <!--Form group with dynamic adding and removal of inputs-->
                ...
                <div class="input-group entry col-sm-10 attribute_input">'
                    ...
                    '<input type="text" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_value_input" placeholder="Value" ' . ((is_array($element->attribute) && sizeOf($element->attribute)==1)?'value='. "'" . (string)$element->attribute[0] . "'" .'':'') . '>
        ...
        //Generate html element with a single input field for xml attribute or value and a button to dynamically add additional copies of this element
            ...
            <div class="form-group row no-gutters justify-content-end">
                ...
                <div class="input-group entry col-sm-10">
                    <input type="text" name="'. $element->prefix . $element->name .'" class=" form-control form-control-sm inputField attribute_input" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_input" placeholder="Attribute">
                    <button class="btn btn-success btn-add" type="button" tabindex="-1">
                        <i class="fas fa-plus"></i>
                    </button>
                </div>
                 ...
                 <div class="col">   
                    <div class="form-group row no-gutters justify-content-end"> 
                        ...
                        <div class="input-group col-12 no-padding">
                            <input type="text" name="'. $element->prefix . $element->name .'" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_attribute_input" ' . ((is_array($element->attribute))?'value='. "'" . (string)$element->attribute[0] . "'" .'':'value='. "'" . (string)$element->attribute . "'" .'') . ' placeholder="Attribute"' . (($element->readOnly==true)?'disabled="disabled"':'') . '>
                            <input type="text" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_value_input" placeholder="Value" value="">
                        </div>
                        ...
                        <div class="col-12 input-group no-padding">
                            <input type="text" name="'. $element->prefix . $element->name .'" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .'id="'. $element->name .'_attribute_input" ' . ((is_array($element->attribute))?'value='. "'" . (string)$element->attribute[1] . "'" .'':'value='. "'" . (string)$element->attribute . "'" .'') . ' placeholder="Attribute"' . (($element->readOnly==true)?'disabled="disabled"':'') . '>
                            <input type="text" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .'id="'. $element->name .'_value_input" placeholder="Value" value="">
                        </div>
                        </span>
                    </div>
                ...
                <div class="col">
                    <div class="form-group row no-gutters justify-content-end">
                        ...
                        <div class="input-group col-12">
                            <input type="text" name="'. $element->prefix . $element->name .'" class="form-control form-control-sm inputField attribute_input" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_attribute_input" ' . ' placeholder="Attribute"' . ((is_array($element->attribute))?'value='. "'" . (string)$element->attribute[0] . "'" .'':'value='. "'" . (string)$element->attribute . "'" .'') . (($element->readOnly==true)?'disabled="disabled"':'') . '>
                            <input type="text" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_value_input" placeholder="Value" value="">
                        </div>
                        ...
                        <span class="col-2"></span>
                        <div class="input-group col-12">
                            <input type="text" name="'. $element->prefix . $element->name .'" class="form-control form-control-sm inputField attribute_input" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_attribute_input" ' . ' placeholder="Attribute"' . ((is_array($element->attribute))?'value='. "'" . (string)$element->attribute[1] . "'" .'':'value='. "'" . (string)$element->attribute . "'" .'') . (($element->readOnly==true)?'disabled="disabled"':'') . '>
                            <input type="text" class="form-control form-control-sm inputField" '.(($element->value=='date')?'':'onfocus="expandInput(this)"') .' id="'. $element->name .'_value_input" placeholder="Value" value="">
                        </div>
                        </span>
                    </div>
